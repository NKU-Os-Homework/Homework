...
{
    /*LAB3 EXERCISE 4: YOUR CODE 2113021 */
...
_clock_map_swappable(struct mm_struct *mm, uintptr_t addr, struct Page *page, int swap_in)
{
    ...
    // 将当前页面加入到当前访问的前一个，处于顺序访问的最后一位
    list_add_before(curr_ptr, entry);
    pte_t *p = get_pte(mm->pgdir, addr, 0);
    *p |= PTE_A;
    return 0;
}
/*
...
_clock_swap_out_victim(struct mm_struct *mm, struct Page **ptr_page, int in_tick)
{
    ...
    while (1)
    {
        ...
        if (curr_ptr != head)
        {
            struct Page *tmp = le2page(curr_ptr, pra_page_link);
            ...
            else
            {
                *ptr_page = le2page(curr_ptr, pra_page_link);
                // 需要将curr_ptr的地址输出才能通过make grade
